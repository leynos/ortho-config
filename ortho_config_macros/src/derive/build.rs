...
///
/// Every field is wrapped in `Option<T>` so we can tell whether the
/// user supplied a value on the command line. An extra `config_path`
/// field is always added to support overriding the configuration file
/// location.

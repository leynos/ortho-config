...
fn build_discovery_tokens(
    ...
    struct_attrs
        ...
        .map(|attrs| DiscoveryTokens {
            ...
            project_root_from: project_root_tokens(fields, attrs.project_root_from.as_deref()),
        })
}

fn project_root_tokens(
    fields: &[syn::Field],
    project_root_from: Option<&str>,
) -> Option<(String, bool)> {
    let field_name = project_root_from?;
    let is_optional = fields
        ...
        .is_some_and(|field| derive::parse::option_inner(&field.ty).is_some());
    Some((String::from(field_name), is_optional))
}
...
fn build_macro_components(args: &MacroComponentArgs<'_>) -> syn::Result<MacroComponents> {
    ...
    let discovery_tokens = build_discovery_tokens(struct_attrs, ident, fields);

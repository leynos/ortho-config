...
mod tests {
    ...
    fn generate_cli_struct_emits_expected_tokens() -> Result<()> {
        ...
        let tokens = generate_cli_struct(&config_ident, &components);
        let expected = quote! {
            #[doc = "CLI parser struct generated by `OrthoConfig` for `Config`."]
    ...
    fn generate_defaults_struct_supports_empty_fields() -> Result<()> {
        ...
        let config_ident = parse_str("Config").context("config ident")?;
        let tokens = generate_defaults_struct(&config_ident, &components);
        let expected = quote! {
            #[doc = "Defaults storage struct generated by `OrthoConfig` for `Config`."]

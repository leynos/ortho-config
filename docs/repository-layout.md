...
| `scripts/`              | Python and shell-adjacent maintenance scripts, including `generate_typos_config.py`, which regenerates the root `typos.toml` spelling configuration. Script tests live under `scripts/tests/` and run through the workspace test target.                                                   |

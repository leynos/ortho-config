...
Next, define a top-level enum where each variant corresponds to one subcommand.
Each variant will hold an instance of the argument struct defined in the
previous step. This enum will also derive `clap::Parser` (if it's the top-level
CLI definition) or `clap::Subcommand` (if it's part of a larger `clap`
structure).
...
subcommand argument struct (e.g., `EncodeArgs`, `DecodeArgs`). This section
holds the specific logic for each subcommand.

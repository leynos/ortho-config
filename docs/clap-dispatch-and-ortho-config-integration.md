...
Ensure the Rust project is set up with the necessary dependencies. The project
requires `clap` for argument parsing (specifically with the `derive` feature
for ergonomic struct-based parsing) and `clap-dispatch` itself.

Add the following snippet to `Cargo.toml`:
...
The command `cargo add clap -F derive` adds `clap` with the derive
feature.[^clap] `clap-dispatch` itself depends on crates like `syn`, `quote`,
and `proc-macro2` for its procedural macro functionality, but these are
transitive dependencies and do not need to be added explicitly by application
authors.[^clap-dispatch]

### B. Defining Argument Structs with `clap::Parser`

For each subcommand, define a Rust struct to hold its specific arguments and
options. This struct should derive `clap::Parser`, allowing `clap` to
automatically generate the parsing logic.
...
These structs (`EncodeArgs`, `DecodeArgs`) encapsulate the arguments unique to
the `encode` and `decode` subcommands, respectively. This is a standard
approach when using `clap`'s derive API.[^clap]

### C. Structuring CLI with an Enum for Subcommands

Next, define a top-level enum where each variant corresponds to a subcommand.
Each variant holds an instance of the argument struct defined in the previous
step. This enum also derives `clap::Parser` (if it's the top-level CLI
definition) or `clap::Subcommand` (if it's part of a larger `clap` structure).
...
With the trait generated by `clap-dispatch`, implement this trait for each
subcommand's argument struct (e.g., `EncodeArgs`, `DecodeArgs`). This is where
the specific logic for each subcommand resides.
...
Finally, in `main.rs`, parse the command-line arguments using
`clap::Parser::parse()` on the main CLI enum. The dispatched function (for
example `process`) can then be called directly on the parsed enum instance.
...
impl Execute for ListItemsArgs { fn execute(&self, db_connection_url: &str) ->
Result<(), String> { let pool = DbPool::new(db_connection_url);
pool.list_items(self.category.as_ref(), self.all); Ok(()) } }

// 4. Main function to parse and dispatch fn main() -> Result<(), String> { let
cli_instance = RegistryCommands::parse(); let db_url =
"postgres://user:pass@localhost/registry"; cli_instance.execute(db_url) }
````
...
does not impose restrictions on the subcommand's own arguments; it standardises

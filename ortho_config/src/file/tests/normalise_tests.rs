//! Tests validating `normalize_cycle_key` behaviour across platforms.

use super::super::path::normalize_cycle_key;
use super::assert_normalize_cycle_key;
...
#[test]
fn normalize_cycle_key_is_noop_on_case_sensitive_platforms() -> Result<()> {
    let path = PathBuf::from("/tmp/Config.toml");
    let normalised = normalize_cycle_key(&path);
...
fn normalize_cycle_key_case_insensitive_scenarios(
    #[case] windows_input: &str,
    #[case] windows_expected: &str,
    #[case] unix_input: &str,
    #[case] unix_expected: &str,
) -> Result<()> {
    assert_normalize_cycle_key(windows_input, windows_expected, unix_input, unix_expected)?;
    Ok(())
}
...
#[test]
fn normalize_cycle_key_handles_unicode_and_special_characters() -> Result<()> {
    if cfg!(windows) {
        let unicode = PathBuf::from(r"C:\\Temp\\CAFÉ.toml");
        let special = PathBuf::from(r"C:\\Temp\\Config-!@#.toml");
        ensure!(
            normalize_cycle_key(&unicode) == Path::new(r"c:\\temp\\cafÉ.toml"),
            "unexpected unicode normalisation"
        );
        ensure!(
            normalize_cycle_key(&special) == Path::new(r"c:\\temp\\config-!@#.toml"),
        ...
        let special = PathBuf::from("/tmp/Config-!@#.toml");
            ...
            normalize_cycle_key(&unicode) == Path::new("/tmp/café.toml"),
            "unexpected unicode normalisation"
        );
        ensure!(
            normalize_cycle_key(&special) == Path::new("/tmp/config-!@#.toml"),
            "unexpected special character normalisation"
        );
    }
    Ok(())

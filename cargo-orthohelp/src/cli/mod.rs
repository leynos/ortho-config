...
/// Enforcement mode for the `--check-agent-native` lint.
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum CheckMode {
    /// Do not evaluate agent-native policy checks.
    Off,
...
pub struct Args {
    ...
    /// Run the agent-native behaviour lint with the given enforcement mode.
    #[arg(
        long = "check-agent-native",
        value_enum,
        num_args = 0..=1,
        require_equals = true,
...
impl From<CheckMode> for cargo_orthohelp::policy::PolicyMode {
    fn from(mode: CheckMode) -> Self {
        match mode {
            CheckMode::Off => Self::Off,
            CheckMode::Warn => Self::Warn,
            CheckMode::Deny => Self::Deny,
        }
    }
}

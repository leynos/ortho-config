...
//! (`Option < bool >`). Agents consume these as opaque literals, so the
//! whitespace is collapsed while every string, character, and raw-string
//! literal is preserved byte for byte.
